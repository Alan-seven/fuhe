...
@Controller
@RequestMapping(value = "${adminPath}/emergency/test")
public class TestController extends BaseController{
	...
	public static final int PORT = 8123;//监听的端口号     
	/**
	 * 调度结果
	 * @param ddsEdPVO
	 * @param response
	...
	public String initResult(DdsEdPVO ddsEdPVO, HttpServletRequest request,HttpServletResponse response){
		...
		if(null!=ddsEdP.getSta()&& ddsEdP.getSta()>=4){//如果模型已经演算完成
			...
			//得到水库调节设置初始条件
			 ...
			 forTimeCal.set(Calendar.HOUR, -foreTime);
			...
			// conditions.setBG_CONC(BG_CONC);
			 ...
			 Future<ModelResult> future = this.executorService.submit(
	                    new Callable<ModelResult>(){

	                        @Override
	                        public ModelResult call() throws Exception {
	                        	ModelResult mr = new ModelResult();
	                            	// mr = calmodel.startModel(conditions,reserviors,states,mResult);
	                            	 mResult.setInformation("");
	                            	return mr;
	                        }
	                    });
			 ...
			 if (!future.isCancelled()) { 
				 if (!future.isCancelled()) { 
	            	while(!future.isDone()){
								//System.out.println("执行未完成:"+future.get().getInformation());
	            		String information = mResult.getInformation()==null?"正在准备数据":mResult.getInformation();
	            		HttpSession session = request.getSession();
	            		session.setAttribute(ddsEdPVO.getProCd(), information);
				 ...
				 if (future.isDone()) {  
						...
						try {
							ModelResult result = future.get();
							// 判断结果  
		                    if (result != null) {  
		                    	HttpSession session = request.getSession();
			            		session.setAttribute(ddsEdPVO.getProCd(), "执行成功");
			            		logger.warn("执行成功");
