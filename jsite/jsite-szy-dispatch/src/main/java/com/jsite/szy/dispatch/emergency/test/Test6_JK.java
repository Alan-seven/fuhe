...
public class Test6_JK {
    ...
    public static void main(String[] args) {
        ...
        double LEN_UP=2; //起点距 = 距离上监测断面的距离+上断面的起点距   总长115.39m
        double DA=50;// 污染量 kg
         ...
         */
         double BG_CO=0;
         double[] BG_Z={66.75};//水库初始水位（水库运行方式界面）  江口水库
         //double[] BG_CONC={0,0}; // 水库初始污染物浓度（新建方案界面）-暂时没用
         double[] BG_Q={0};// 测站初始流量（新建方案界面）-暂时没用

         double[] WQgoal={0.01,0.01,0.01,0.01,0.01,0.01,0.01}; // rcd=6时，有7个控制断面
         ...
         */
         ...
         for(int i=0;i<foreTime;i++){
             outflow[0][i]=116;//江口出库
             deltaflow[0][i]=0;//江口加大出库
            }
         // 模拟期输入从界面设置获取
         for(int i=foreTime;i<outflow[0].length;i++){
             outflow[0][i]=116;//江口出库
             deltaflow[0][i]=10;//江口加大出库
            }
