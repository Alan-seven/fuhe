...
public class TestSZ {
	...
	public static void main(String[] args) {
        ...
        int RCD=1;//发生河道
        String UPSEC="02000001";//上监测断面ID-如：砂子岭
        double LEN_UP=82.69; //起点距 = 距离上监测断面的距离+上断面的起点距（河段1全长95.36）
        double DA=2022.99;// 污染量 kg
         ...
         */
        //String PRO_CD; 
        int foreTime=3;//前沿时段：小时 ——数据库加
        //Date startTime;// 计算起始时间， 污染发生时间 >= startTime - foreTime
        int deltaT=1;//输入序列的时间间隔：1h
        int length=48;//计算总时段数，单位：h
        int accurTime=0;//污染发生时间 距离 计算起始时间的时段数 (TM-startTime)/deltaT
        int durTime=5; // 污染物持续时段数 DUR/deltaT
         ...
         */
         double BG_CO=0;
         //double[] BG_Z={111.9,86,63};//水库初始水位（水库运行方式界面）
         //double[] BG_CONC={0,0}; // 水库初始污染物浓度（新建方案界面）-暂时没用
         double[] BG_Q={0,0};// 测站初始流量（新建方案界面）-暂时没用
     
         double[] WQgoal={0.01,0.01,0.01}; // rcd=1时，有3个控制断面
       ...
       for(int i=0;i<foreTime;i++){
             boundry[0][i]=30.6;//砂子岭流量
             boundry[1][i]=63.5;//南城水位
