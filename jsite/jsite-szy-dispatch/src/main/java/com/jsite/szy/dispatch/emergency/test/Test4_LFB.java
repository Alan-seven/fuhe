...
public class Test4_LFB {
    ...
    public static void main(String[] args) {
        ...
        double LEN_UP=66; //起点距 = 距离上监测断面的距离+上断面的起点距
        double DA=50;// 污染量 kg
         ...
         * 方案边界条件信息表 
         */
         double[][]boundry=new double[5][foreTime+length+1]; //根据模型配置的顺序，每一个边界存一个数组-界面传
         //预热期输入读历史实际值
         for(int i=0;i<foreTime;i++){
             boundry[0][i]=235;//洪门入库
             boundry[1][i]=116;//廖坊入库
             boundry[2][i]=4.5;//娄家村汇入
             boundry[3][i]=23.16;//马圩汇入
            // boundry[4][i]=23.8;//李家渡水位
            }
         // 模拟期输入从界面设置获取
