...
import com.jsite.manager.role.DataScope;
...
public abstract class AbstractBaseService {
	 ...
	 * @return 标准连接条件对象
	 */
	public static String dataScopeFilter(DdsSysUserinfo user, String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()) {/*
			...
			for (Role r : user.getRoleDOList()) {
				for (String oa : StringUtils.split(officeAlias, ",")) {
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)) {
						if (DataScope.DATA_SCOPE_ALL.equals(r.getDataScope())) {
							isDataScopeAll = true;
						} else if (DataScope.DATA_SCOPE_COMPANY_AND_CHILD.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getCompany().getId() + "'");
							sqlString.append(" OR " + oa + ".parent_ids LIKE '" + user.getCompany().getParentIds()
									+ user.getCompany().getId() + ",%'");
						} else if (DataScope.DATA_SCOPE_COMPANY.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getCompany().getId() + "'");
							// 包括本公司下的部门 （type=1:公司；type=2：部门）
							sqlString.append(" OR (" + oa + ".parent_id = '" + user.getCompany().getId() + "' AND "
									+ oa + ".type = '2')");
						} else if (DataScope.DATA_SCOPE_OFFICE_AND_CHILD.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getId() + "'");
							sqlString.append(" OR " + oa + ".parent_ids LIKE '" + user.getOffice().getParentIds()
									+ user.getOffice().getId() + ",%'");
						} else if (DataScope.DATA_SCOPE_OFFICE.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getId() + "'");
						} else if (DataScope.DATA_SCOPE_CUSTOM.equals(r.getDataScope())) {
							// String officeIds =
						...
						// else if
						// (IRole.DATA_SCOPE_SELF.equals(r.getDataScope())){
	 ...
	 */
	public static void dataScopeFilter(AbstractBase<?> entity, String sqlMapKey, String officeWheres,
			String userWheres,HttpServletRequest request) {
		...
		/*for (String where : StringUtils.split(officeWheres, ",")) {
			if (DataScope.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)) {
				// 包括本公司下的部门 （type=1:公司；type=2：部门）
				...
				sqlString.append(" OR parent_ids LIKE '" + user.getCompany().getParentIds()
						+ user.getCompany().getId() + ",%')");
				sqlString.append(" AND " + where + ")");
			} else if (DataScope.DATA_SCOPE_COMPANY.equals(dataScopeString)) {
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
				sqlString.append(" WHERE type='2'");
				sqlString.append(" AND id = '" + user.getCompany().getId() + "'");
				sqlString.append(" AND " + where + ")");
			} else if (DataScope.DATA_SCOPE_OFFICE_AND_CHILD.equals(dataScopeString)) {
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
				sqlString.append(" WHERE (id = '" + user.getOffice().getId() + "'");
				sqlString.append(" OR parent_ids LIKE '" + user.getOffice().getParentIds()
						+ user.getOffice().getId() + ",%')");
				sqlString.append(" AND " + where + ")");
			} else if (DataScope.DATA_SCOPE_OFFICE.equals(dataScopeString)) {
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
				sqlString.append(" WHERE id = '" + user.getOffice().getId() + "'");
				sqlString.append(" AND " + where + ")");
			} else if (DataScope.DATA_SCOPE_CUSTOM.equals(dataScopeString)) {
		...
		for (String where : StringUtils.split(userWheres, ",")) {
			if (DataScope.DATA_SCOPE_SELF.equals(dataScopeString)) {
