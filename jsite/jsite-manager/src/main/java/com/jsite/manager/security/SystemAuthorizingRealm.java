package com.jsite.manager.security;
...
import com.jsite.core.servlet.ValidateCodeServlet;
import com.jsite.core.spring.SpringContextHolder;
import com.jsite.core.utils.BooleanUtils;
...
import com.jsite.manager.user.LoginValidation;
import com.jsite.manager.user.UserUtils;

/**
 * 系统安全认证实现类  ---  集成水资源单点不需要此验证
 */
@Service
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验登录验证码
		/*if (LoginValidation.isValidateCodeLogin(token.getUsername(), false, false)) {
		...
		if (user != null) {
			if (BooleanUtils.toBoolean(user.getLoginFlag())) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		// 获取当前已登录的用户  未加入单点登录，先注释掉
		/*if (!BooleanUtils.toBoolean(Global.getConfig("user.multiAccountLogin"))) {
		...
		if (user != null) {
			...
			// 添加用户角色信息
			for (Role role : user.getRoleDOList()) {
				info.addRole(role.getEnname());
			}
			// 更新登录IP和时间
			getSystemService().updateUserLoginInfo(user);
			// 记录登录日志
			LogDaoUtils.save(Servlets.getRequest(), "系统登录");
			return info;
		} else {
			return null;
		}*/
