...
import com.jsite.busi.szy.evaluation.dao.DdsWeWuDao;
import com.jsite.busi.szy.evaluation.po.DdsWeWrcs;
import com.jsite.busi.szy.evaluation.po.DdsWeWu;
...
public class DdsWeWuService extends AbstractCrudService<DdsWeWuDao, DdsWeWu> {
    @Autowired
    DdsWeWuDao ddsWeWuDao;
    ...
    public Map<String, List<Map>> listAllToMap_name(String river) {
        ...
        List<Map> dataList = ddsWeWuDao.listAllToMap(regCds);
    ...
    public XSSFWorkbook getTemplate(String river) {
        String[][] headName = new String[][]{
            {
                "分区代码", "分区名称", "年份",
                "农田灌溉用水量（亿m³）", "林牧渔用水量（亿m³）", "工业用水量（亿m³）",
                "城镇公共用水量（亿m³）", "居民生活用水量（亿m³）", "生态环境用水量（亿m³）"
            },
            {"20", "20", "20", "20", "20", "20", "20", "20", "20"}
        };

        XSSFWorkbook wb = new XSSFWorkbook();   //--->创建了一个excel文件
        XSSFSheet sheet = wb.createSheet("用水量评价数据模板");    //--->创建了一个工作簿

        // 列头
        ExcelService.initHeadSheet(wb, sheet, headName, 1);
        JsonNode node = ExcelService.readJson("wceTemp.json");
    ...
    public boolean uploadData(MultipartFile file, String river) {
        ...
        XSSFSheet sheet;
        List<DdsWeWu> dataList = new ArrayList<>();
            ...
            if (sheet != null) {
                ...
                for (int rowNum = firstRowNum; rowNum <= lastRowNum; rowNum++) {
                    ...
                    if (row.getCell(3) != null) {
                        d.setIrrGwWw(Double.valueOf(
                                row.getCell(3).toString().trim()));
                    }
                    if (row.getCell(4) != null) {
                        d.setFfGwWw(Double.valueOf(
                                row.getCell(4).toString().trim()));
                    }
                    if (row.getCell(5) != null) {
                        d.setIndGwWw(Double.valueOf(row.getCell(5).toString().trim()));
                    }
                    if (row.getCell(6) != null) {
                        d.setPGwWw(Double.valueOf(
                                row.getCell(6).toString().trim()));
                    }
                    if (row.getCell(7) != null) {
                        d.setDGwWw(Double.valueOf(row.getCell(7).toString().trim()));
                    }
                    if (row.getCell(8) != null) {
                        d.setEGwWw(Double.valueOf(row.getCell(8).toString().trim()));
                    }
                    d.setRiver(river);
                    dataList.add(d);
                }
            }
            for (DdsWeWu d : dataList) {
                DdsWeWu temp = this.get(d);
                if (temp != null) {
                    this.update(d);
                } else {
                    this.save(d);
