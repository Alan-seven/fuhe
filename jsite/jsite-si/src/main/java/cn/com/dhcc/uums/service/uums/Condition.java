/**
 * Condition.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis 1.4 Apr 22, 2006 (06:55:48 PDT) WSDL2Java emitter.
 */

package cn.com.dhcc.uums.service.uums;

public class Condition implements java.io.Serializable {
    private java.lang.String _value_;
    private static java.util.HashMap _table_ = new java.util.HashMap();

    // Constructor
    protected Condition(java.lang.String value) {
        _value_ = value;
        _table_.put(_value_,this);
    }

    public static final java.lang.String _EQ = "EQ";
    public static final java.lang.String _LIKE = "LIKE";
    public static final java.lang.String _LT = "LT";
    public static final java.lang.String _GT = "GT";
    public static final java.lang.String _LE = "LE";
    public static final java.lang.String _GE = "GE";
    public static final java.lang.String _ISNULL = "ISNULL";
    public static final java.lang.String _ISNOTNULL = "ISNOTNULL";
    public static final java.lang.String _IN = "IN";
    public static final java.lang.String _BETWEEN = "BETWEEN";
    public static final Condition EQ = new Condition(_EQ);
    public static final Condition LIKE = new Condition(_LIKE);
    public static final Condition LT = new Condition(_LT);
    public static final Condition GT = new Condition(_GT);
    public static final Condition LE = new Condition(_LE);
    public static final Condition GE = new Condition(_GE);
    public static final Condition ISNULL = new Condition(_ISNULL);
    public static final Condition ISNOTNULL = new Condition(_ISNOTNULL);
    public static final Condition IN = new Condition(_IN);
    public static final Condition BETWEEN = new Condition(_BETWEEN);
    public java.lang.String getValue() { return _value_;}
    public static Condition fromValue(java.lang.String value)
          throws java.lang.IllegalArgumentException {
        Condition enumeration = (Condition)
            _table_.get(value);
        if (enumeration==null) throw new java.lang.IllegalArgumentException();
        return enumeration;
    }
    public static Condition fromString(java.lang.String value)
          throws java.lang.IllegalArgumentException {
        return fromValue(value);
    }
    public boolean equals(java.lang.Object obj) {return (obj == this);}
    public int hashCode() { return toString().hashCode();}
    public java.lang.String toString() { return _value_;}
    public java.lang.Object readResolve() throws java.io.ObjectStreamException { return fromValue(_value_);}
    public static org.apache.axis.encoding.Serializer getSerializer(
           java.lang.String mechType, 
           java.lang.Class _javaType,  
           javax.xml.namespace.QName _xmlType) {
        return 
          new org.apache.axis.encoding.ser.EnumSerializer(
            _javaType, _xmlType);
    }
    public static org.apache.axis.encoding.Deserializer getDeserializer(
           java.lang.String mechType, 
           java.lang.Class _javaType,  
           javax.xml.namespace.QName _xmlType) {
        return 
          new org.apache.axis.encoding.ser.EnumDeserializer(
            _javaType, _xmlType);
    }
    // Type metadata
    private static org.apache.axis.description.TypeDesc typeDesc =
        new org.apache.axis.description.TypeDesc(Condition.class);

    static {
        typeDesc.setXmlType(new javax.xml.namespace.QName("http://uums.service.uums.dhcc.com.cn/", "condition"));
    }
    /**
     * Return type metadata object
     */
    public static org.apache.axis.description.TypeDesc getTypeDesc() {
        return typeDesc;
    }

}
